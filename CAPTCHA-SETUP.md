...
网站已经接入 Cloudflare Turnstile，并会把验证令牌交给 Supabase Auth 在服务端校验。还需要站长完成下面的密钥配置，登录、注册和找回密码入口才会开放。

## 1. 创建 Turnstile Widget

1. 登录 [Cloudflare Dashboard](https://dash.cloudflare.com/)，进入 **Turnstile**。
2. 点击 **Add widget**，名称可填写 `cnbdg-blog-auth`。
...
Supabase 开启保护后，没有有效 Turnstile 令牌的登录、注册和密码重置请求会被服务端拒绝，不能只靠修改浏览器前端绕过。

## 4. 验证效果

1. 等待 GitHub Pages 部署完成，使用无痕窗口打开 `https://cnbdg.co/`。
2. 分别进入登录页和注册页，确认“安全验证”可以正常加载。
3. 未完成验证时登录或注册按钮应保持禁用；验证完成后按钮会启用。
4. 测试登录、注册和“忘记密码”，确认不再出现 `no captcha_token found`。
